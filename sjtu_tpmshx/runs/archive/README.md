...
**死路径声明 (P1.7, 2026-07-20)**: 本目录脚本里写死的 `D:/Postgraduate/vault/...`
输出路径是旧开发机布局, 本机不可解析——**故意不改**: 它们是"当年在哪、怎么跑出来"
的证据链一部分。要重跑请复制到 `runs/diagnostics/` 改路径; 活跃工具的路径规范
(env 覆盖 + `runs/_out` 默认) 见 P1.7 注释。
