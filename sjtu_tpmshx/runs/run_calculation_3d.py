...
def _parse_inputs(window):
    ...
    return dict(
        ...
        # B's mass flux is < 20% of the p75 reference) get full LTNE
        # coupling to the hot solid → T_b equilibrates to T_a everywhere
        # → cold region collapses to just the inlet pipe footprint.
        # Method H8 (mass_flux_threshold) is geometry-independent (no
        # u_ref tuning).
        #
        # 2026-05-07 bugfix: previous commit defaulted threshold_frac=0.05
        # which produced part_frac=1.000 (all cells passing) because the
        # 5% cutoff is below the natural Brinkman-Forchheimer leakage
        # floor. Production config in vault memory
        # `feedback_h8_per_cell_chi_b` is thr=0.20, n_dilate=1.
