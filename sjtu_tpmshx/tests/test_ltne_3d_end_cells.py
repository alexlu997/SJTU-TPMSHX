...
def _check_two_cells(direction, fraction, mode, unequal):
    ...
    for base, kval, exchange, flux, tin, frac in (
        (0, ka, ha, fa, 360.0, fraction), (2, kb, hb, fb, 300.0, 1.0)
    ):
    ...
    if cc_mode:
        ...
        try:
            set_num_threads(2)
            fn(*args, one,0.0,one*0,one*0,one*0,int(conservative))
        finally:
    ...
    total = 0.0
    for t,kval,flux,tin,frac in ((ta,ka,fa,360,fraction),(tb,kb,fb,300,1)):
        f0,fm,f2 = flux
...
def test_actual_inlet_transport_has_no_second_area_factor(direction):
    ...
    rcp = np.arange(8).reshape(2,2,2)+4.0
    ...
    actual = _inlet_transport_3d(faces,eps,rcp,*widths,direction)
    ...
    for a,b in np.ndindex(2,2):
        ...
        expected = (eps[tuple(cell)]*rcp[tuple(cell)]*(0.8,1.2,1.6)[axis]
                    *widths[transverse[0]][a]*widths[transverse[1]][b]
                    *(1 if direction%2 == 0 else -1))
        assert actual[a,b] == pytest.approx(expected)


def test_prescribed_b_is_external_reservoir_and_not_a_certificate():
    ...
    expected = np.linalg.solve(matrix,[(din+flux)*360,0,hb[0]*300,hb[1]*300])
    ...
    a_boundary = 2*(flux*(360-expected[1])+din*(360-expected[0]))
    assert external_b < 0
