"""plot_nu_loo_4p.py — Re vs Nu LOO plot for 4p simple form.

Form: Nu = c · Pr^(1/3) · exp(a2·(ln Re)²) · ε_f^b · (D_h/L)^d
"""
...
def loo_predictions(tpms):
    ...
    p0 = [1, 0.04, -0.5, 0.5]
    bounds = ([1e-5, -0.5, -10, -10], [100, 0.5, 10, 10])
    for L_t, t_t in geoms:
        ...
        try:
            popt, _ = curve_fit(nu_4p, X_tr, d_tr['Nu'].values,
        ...
        Nu_pred[d.index[sel_test].to_numpy()] = nu_4p(X_te, *popt)
    return d, Nu_pred
...
def plot_panel(ax_curve, ax_resid, d, Nu_pred, title):
    ...
    ax_curve.set_xlabel(r'$Re$ (Excel design)')
...
def main():
    try:
        ...
        pass
    print("Generating Nu 4p LOO error figure (v3 CFD4, Excel Re)")
    print("=" * 60)
    print("Diamond LOO...")
    ...
    rd, bd = plot_panel(axes[0, 0], axes[1, 0], dD, predD, 'Diamond (4p)')
    rg, bg = plot_panel(axes[0, 1], axes[1, 1], dG, predG, 'Gyroid (4p)')

    fig.suptitle('Nu 4p LOO interpolation — v3 CFD4, '
                 r'$Nu = c\cdot Pr^{1/3}\cdot \exp(a_2(\ln Re)^2)\cdot \varepsilon_f^b\cdot (D_h/L)^d$'
                 ' (smooth-wall, no ×1.28)',
                 fontsize=12, y=0.995)
    ...
    out_png = out_dir / 'nu_loo_error_4p_v3.png'
    fig.savefig(out_png, dpi=150, bbox_inches='tight')

    vault = Path(r'D:\Postgraduate\vault\reports\methodology\figs')
    vault.mkdir(parents=True, exist_ok=True)
    fig.savefig(vault / 'nu_loo_error_4p_v3.png', dpi=150, bbox_inches='tight')

    print(f"\nSaved: {out_png}")
    print(f"Saved: {vault / 'nu_loo_error_4p_v3.png'}")
    print(f"\nDiamond 4p LOO RMSRE = {rd:.2f}% bias = {bd:+.2f}%")
    print(f"Gyroid  4p LOO RMSRE = {rg:.2f}% bias = {bg:+.2f}%")
