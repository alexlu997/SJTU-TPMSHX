...
4. **编码**：源码含大量中文字符串/docstring（如 `sjtu_tpmshx/ui/builders_sidebar.py:1`、`sjtu_tpmshx/ui/quick_design_panel.py:1`、`sjtu_tpmshx/ui/builders_canvas.py:500`–`501`），文件本身 UTF-8；Linux 上确保 `LANG`/`PYTHONIOENCODING` 为 UTF-8（`demo_vis_3d.py:30` 有 `sys.stdout.reconfigure(encoding='utf-8')` 自救，其他入口没有）。
5. **字体假设**：`apply_mpl_theme` 的 sans 栈顺序为 `Fira Sans`/`Inter` 优先，其后 `Segoe UI`/`Helvetica`/`Arial`（+ Segoe UI Symbol/Emoji），`DejaVu Sans` 兜底（`sjtu_tpmshx/ui/theme.py:546`–`548`）——Linux 上未装前列字体时自动落到 DejaVu，功能不受影响但观感变化；QSS 中的 Fira Code 等同理（`sjtu_tpmshx/ui/theme.py:304` 附近注释）。
...
8. **平台特定代码**：`glass_panel.py` 的噪点混合注释假设 QImage 内存序为 "BGRA on Windows"（`sjtu_tpmshx/ui/glass_panel.py:78`）——`Format_RGB32` 在小端 Linux 上同为 BGRA 布局，通常无碍，但属未在 Linux 实测的假设（未验证）；纯装饰用途，异常时整段 try/except 跳过（:84–:85）。
9. **已知 CI 教训**（来自项目记忆，代码位置在 main.py/CI 配置侧，本篇未逐行核实）：Linux fresh checkout 首启的 onboarding 弹窗曾挂死 CI，需 offscreen guard + 超时；日志行里的中文在 GBK 终端下会污染 pytest capture——服务器统一 UTF-8 locale 可规避。
