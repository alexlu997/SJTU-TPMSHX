...
> 目标平台是 **Windows Server 2022**，不是 Linux（此前写本节时误判了目标平台，以下已按 Windows→Windows Server 重新核实）。

- **本层整体依赖 PySide6 GUI**，无交互式会话时（Windows Server Core、以计划任务/服务方式跑批、未登录的 RDP 会话）需 `QT_QPA_PLATFORM=offscreen`；这是 Qt 自身跨平台的离屏平台插件机制，Windows 与 Linux 通用，不是"仅 Linux 有效"或需要 X11/Wayland 特定处理的东西——判定代码是纯字符串比较（`_os.environ.get('QT_QPA_PLATFORM', '').lower() == 'offscreen'`，`builders_canvas.py:535`；`QApplication.instance().platformName() == 'offscreen'`，`main.py:1014`），不含任何平台分支。代码已有两处 offscreen 感知：跳过 3D PyVista 面板（`builders_canvas.py:535-538`）与跳过首启模态（`main.py:1014`）。matplotlib 后端在 import 早期硬设 `matplotlib.use("QtAgg")`（`main.py:18`）——headless 复用任何会 import `main` 的代码都会拉起 Qt 依赖，这一点与目标平台无关。
...
- **进程自重启用 `os.execv(sys.executable, [sys.executable] + sys.argv)`**（`appearance.py:62/112/183`）：这条不再是"移植到新平台"的风险——代码从写下时起就跑在 Windows 上，且已有注释自证可行（"Works on Windows via the CRT shim"，`appearance.py:179-181`）：Windows 上的 `execv` 由 CRT 用 spawn-新进程+当前进程退出模拟，并非 POSIX 意义上原地替换进程映像，但对外表现等价（旧 PID 退出、新 PID 接手）。包装脚本/监控按此语义容忍 PID 切换即可，Windows Server 与开发机上的 Windows 行为一致，无需额外处理。
