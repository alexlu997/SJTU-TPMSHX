...
- **`evaluate_3d(x_decision, cfg, *, Nx=40, Ny=16, Nz=16, Lz=0.042, max_outer=3, outer_tol_K=0.5, alpha_outer=0.6, max_iter_simple=800, tol_simple=1e-2, max_iter_energy=2000, tol_energy=0.5, roughness_mode=None, roughness_eps_um=None, verbose=True) -> dict`**（`core/evaluators.py:108-122`）— 对单个决策向量运行 3D LTNE 评估，返回键 `Q_3D_W` / `dP_A_Pa` / `dP_B_Pa` / `dP_total_Pa` / `mass_kg` / `Lz_m` / `grid`（`core/evaluators.py:425-433`）。注意 **`cfg` 是原始 dict 而非 `ComputeConfig`**，键名为 `L_domain`/`H_domain`/`u_A`/`u_B`/`T_inA` 等（`core/evaluators.py:126-136`），与 `ComputeConfig` 命名体系不同。调用方：`optimization/evaluator_3d.py:36`、`validation/cases/verify_pareto_3d.py:47`。
...
- **编码安全**：三包所有 JSON I/O 显式 `encoding='utf-8'`（`configs/__init__.py:40`、`domain/compute_config.py:395`、:401-404），Linux 默认 locale 下无问题；`shanghai_baseline.json` 的 `_meta` 含中文字符串（`configs/shanghai_baseline.json:3-7`），终端打印时需 UTF-8 环境（仓库已知 GBK 中文日志会污染 pytest capture，见 memory；本三包自身无 print）。
- **无 Windows 路径硬编码**：三包全部走 `pathlib`（`configs/__init__.py:13`、`domain/compute_config.py:110`）；`shanghai_baseline.json:5` 的 `data/raw_data/...xlsx` 仅是出处注记，加载函数不读该文件（`configs/__init__.py:39-41` 只读同目录 JSON），故 gitignored 大文件缺失不影响本包。
