...
- **原子写依赖 `os.replace` 同文件系统语义**：`session_manager.py:172`；tmp 文件与目标同目录故一般安全；fsync 失败被容忍（`session_manager.py:166-171`），NFS 等虚拟文件系统上语义已有兜底。
- **`pythonw` / 空 stderr 兼容是 Windows 痕迹**：`_Tee` 容忍 `sys.__stderr__ = None`（`compute_orchestrator.py:117-118`, 注释）；Linux 下 `sys.__stdout__` / `__stderr__` 正常存在，无需处理，但注意 tee 写的是 `sys.__stdout__`（原始流）而非当前 `sys.stdout`——若移植方在外层再包一层重定向，捕获不到。
- **编码**：会话/预设读写显式 `encoding='utf-8'`（`session_manager.py:122`, `163`, `246`, `265`），无 GBK 假设，Linux 下无碍。仓库其他层的 GBK 中文日志问题（pytest capture 污染）不在本包。
