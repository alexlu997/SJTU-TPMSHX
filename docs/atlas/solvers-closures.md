...
- **路径/编码**：本模块群无硬编码 Windows 路径、无文件 I/O（除 `fit_cf_aniso.py` 读 CSV，属 validation 脚本）；源码含 UTF-8 中文/希腊字母 docstring（如 `asym_geometry.py`、`df_projection.py`），Linux 下确保 locale 为 UTF-8（`LANG=C.UTF-8`），避免日志/pytest capture 因编码降级（项目 memory 有 GBK 中文日志毒化 pytest capture 的先例，Linux UTF-8 下风险更低）。
