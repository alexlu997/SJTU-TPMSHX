...
- **GeometryLUT 默认把 `.npz` 缓存写进包目录**（`os.path.dirname(os.path.abspath(__file__))`，`sjtu_tpmshx/solvers/sigmoid_field.py:118-119`）。若部署位置对运行账户只读（例如安装进未提权的系统级 site-packages、只读网络共享或不可写的容器/镜像层——Windows Server 上典型场景是以受限服务账户运行、代码装在 `Program Files` 一类需要管理员权限才能写入的目录），首次构建 LUT 会在 `_save` 处抛 `PermissionError`——传 `cache_dir` 指向可写目录（如按运行账户在 `%LOCALAPPDATA%` 下建自定义目录），或预热缓存后随包分发。缓存文件名 `lut_{type}_{n_L}x{n_t}_N{N}.npz`（`sjtu_tpmshx/solvers/sigmoid_field.py:126-127`）。目标 Windows Server 实例的实际安装目录权限未验证，迁移前应实测一次 LUT 首建。
- **`triangle` 是可选依赖**（纯 C 扩展）。PyPI 发行版含 Windows 预编译 wheel（已用 `pip download triangle --no-deps --python-version 3.11 --only-binary=:all: --platform win_amd64` 核实，实际拉到 `triangle-20250106-cp311-cp311-win_amd64.whl`），Windows Server 上 `pip install triangle` 不需要本地编译工具链。缺失时静默走 scipy 回退网格（`sjtu_tpmshx/solvers/unstructured_mesh.py:93-95`），单元数量与质量不同——如需服务器与开发机 polygon 结果逐位对比，两端必须同装或同缺 `triangle`（且宜锁同一版本，未逐一核实跨版本三角化是否比特级一致）。
...
- **环境变量开关在 PowerShell 下同样生效**（`os.environ.get` 读取与调用 shell 无关，`sjtu_tpmshx/solvers/sigmoid_field.py:306-308`）：`TPMSHX_ALLOW_EXTRAP=1` 绕过 L/t clip。PowerShell 里设置用 `$env:TPMSHX_ALLOW_EXTRAP = '1'`（区别于 bash 的 `export`），且该赋值只对当前会话及其子进程生效，不会像 `setx`/系统级环境变量那样持久化到新会话；批量脚本移植（尤其是套用本仓 `scripts/port_retest_server.ps1` 这类现成 Windows Server 变体）时注意不要从旧会话或启动脚本里无意继承该变量，否则 LUT/Nu 静默外推（仅 warning，不报错）。
