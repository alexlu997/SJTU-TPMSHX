...
4. **并行与确定性**：并行 kernel 用 numba `prange`，线程数由 numba 运行时（`NUMBA_NUM_THREADS`）决定；red-black 设计保证结果不随线程数变（同色胞只读异色邻居 + SOU 快照，`sjtu_tpmshx/solvers/_kernels_simple_3d.py:275-279`；`sjtu_tpmshx/solvers/_kernels_ltne_3d.py:597-614`）。golden 位同一门要求 `PYTHONHASHSEED=0`（repo `/check` 约定）。fastmath 位级结果依赖 numba/LLVM 版本与 CPU 指令集——跨平台迁移后 golden hash 大概率不再逐位一致，应按数值容差重基线而非追位同一（推断项：基于 fastmath 语义，未在 Linux 实测）。
5. **平台无关性**：本 7 文件无 Windows 专有路径、无 GUI 依赖、无 GBK 编码操作；`_PARALLEL_CELL_THRESHOLD=200000` 与 `_AMG_GATE=30000` 是 8 核桌面上的经验分界（`sjtu_tpmshx/solvers/simple_solver_3d.py:73-81, 90-96`），服务器核数不同时可经 `TPMSHX_PARALLEL_THRESHOLD` 重调（AMG 门无 env 开关，需改代码）。
...
7. **日志**：经 `logutil.get_logger`（`sjtu_tpmshx/solvers/simple_solver_3d.py:68-70`）；repo 记忆有「日志行含中文（GBK）会毒化 pytest capture」的历史坑——Linux 默认 UTF-8 环境该坑消失，但若设置 `PYTHONIOENCODING` 或非 UTF-8 locale 需注意。
