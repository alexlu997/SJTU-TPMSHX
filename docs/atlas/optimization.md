...
2. **BO 引擎**：`optimizer_qnehvi.py` 用 BoTorch qNEHVI（q-Noisy Expected Hypervolume Improvement）驱动双目标（最大化 Q、最小化 dP）优化，含 Sobol 初始化、GP 拟合、HV 平台早停、Pareto CSV **快照**。
   > ⚠️ 两处措辞更正（codex 审计 2026-07-12，已核实）：
   > ① 那些每 5 轮写出的 `pareto_iter*.csv` / `pareto_latest.csv`（`optimizer_qnehvi.py:449-450`）**不是可恢复的 checkpoint，只是取证快照** —— 全仓无任何代码读回它们，`run_port_dim_retest.py` 的 argparse 也没有 `--resume`。**臂崩了只能从零重跑**（含全部 Sobol init）。
   > ② acquisition function 用的是 `qNoisyExpectedHypervolumeImprovement`（`:217-218`），botorch 0.17.2 在每次实例化时打 `NumericsWarning`，官方判定其"有已知数值问题、导致次优优化性能"，**建议改用 API 完全相同的 `qLogNEHVI`**（arXiv:2310.20708）。既有 HV 数字全部产自这个旧 acqf。
