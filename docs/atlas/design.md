...
- **Excel I/O 用 `openpyxl`**（`cases.py:6`；`report.py` 经 `pandas.ExcelWriter(engine="openpyxl")`，`report.py:58`），跨平台无 Windows 专属依赖；符合仓库约定"Excel: always engine='openpyxl'"。
- **CSV 读取显式用 `encoding="utf-8-sig"`**（`cases.py:72`），用于兼容 Windows Excel 导出 CSV 时的 BOM，Linux 上该编码同样有效，不需要修改。
- **`joblib` + `loky` 后端做跨进程并行**（`select.py:29-34`）：`loky` 是纯 Python 的跨平台进程池实现，但 `sizing.py:16-19` 的注释明确指出 **env var 必须在 import `design.sizing` 之前设置**才能被 spawn 的子进程继承——这一行为在 Linux（`fork`/`spawn` 具体选择由 `loky`/`multiprocessing` 决定）和 Windows（`multiprocessing` 只能 `spawn`，必须重新 import）之间可能有差异。**未验证** loky 在 Linux 上默认使用的 start method；但当前实现（在 import 时经 `os.environ.get` 读一次性冻结到模块全局）在两种 start method 下都能正确工作，因此该注释里的约束（"必须在 import 前设置"）在 Linux 上依然安全，不需要为移植改动代码。
- **无 GUI 强依赖**：`design/` 目录本体不 `import PySide6`/Qt（已用 grep 核实 `design/*.py` 无 `PySide6`/`Qt` 字样）；GUI 集成层在 `sjtu_tpmshx/ui/quick_design_panel.py`，该文件把 `from PySide6.QtCore import QThread, Signal` 放在函数体内懒加载（`ui/quick_design_panel.py:69`），因此 `design/` 包可以在无头 Linux 服务器上单独使用（CLI 或直接 Python API），不需要处理 Qt/显示服务器问题。
- **无 Windows 路径硬编码**：`cases.py`/`sizing.py`/`select.py`/`report.py` 全部用 `os.path.splitext`/相对导入，未见反斜杠或盘符路径字面量；`sizing.py:21-22` 的 `TPMSHX_BUILD_S_MAX` 等环境变量名跨平台通用。
- **依赖 `scipy.optimize`（`brentq`, `minimize`）与 `numpy`**：均为标准跨平台科学计算依赖，无平台特定编译扩展代码路径（**未验证**具体版本锁定情况，需查仓库 `requirements.txt`/`pyproject.toml`，本次审阅未打开该文件）。
- **并行进程数默认 `-1`（全核）**：`select.py` 的 `n_jobs=-1` 语义由 `joblib` 决定（通常映射为可用 CPU 核数），在容器化/CI 环境下可能需要显式限制核数以避免与其他任务抢占 CPU——这是运维层面的注意事项，非代码缺陷。
