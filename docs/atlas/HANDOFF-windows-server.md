...
## ⟨2026-07-20 状态总更新（upgrade/loop 分支收编）⟩

本文写于 2026-07-11，是**证据快照**——下表按问题给出分支上的现状；原文一律不改写（保留
当时的证据链），已解决节只在结论行加戳。证据 commit 均在 `upgrade/loop` 分支（未推送）。

| 节 | 07-11 发现 | 2026-07-20 现状 |
|---|---|---|
| §1 | max_outer 丢弃 + 压力字面量硬编码 | **上游已修**（P1.2 / iter 7 逐条验证，无码可写收案） |
| §2 | optimizer 路径无 envelope gate | **已解**：预解 choke raise→罚值（aa3f477，先于分支）；P1.3-A/B 补齐 envelope 权威统一 + 3D 解后 Mach/正压门；常驻守卫 test_evaluator_envelope_authority |
...
| §9c | 开发机环境快照 | 被锁文件取代（服务器权威口径见锁文件头注） |
| §9e | 建议清单 | 大部分已执行（见上各行） |
| §10 | atlas 分支可否合入 | **已过时**：atlas 早已在 master（本分支基点 4b32da4 含之）；PR #47 撞车问题随之消失 |
| 附注 | AGENTS.md 过期路径（repo-infra 卷旧条目） | **收案**：AGENTS.md 在本分支基点树中不存在（git 历史无记录，系当年开发机未跟踪文件），无可修对象 |

汇总节（"需要你回答的"/"建议优先修的"）请对照上表阅读——多数"优先修"项已由升级循环完成。

---

## 1. `--runner pipeline` 的 `max_outer` 丢弃 + `pressure_clip_hits`/`pressure_state_valid` 硬编码

### 结论：**是遗漏，不是有意忽略。而且第二个问题比第一个严重。** ⟨07-20：两处上游均已修复，P1.2 验证收案——见文首状态表⟩
...
### 结论：**是实现时漏掉了 optimizer 路径，不是有意的 BO 契约设计。** 但"漏掉"的实际风险比听起来小（见下）。⟨07-20：已解——aa3f477 + P1.3-A/B，常驻守卫在案；见文首状态表⟩
...
### 结论：**代码里没有"应该一致"的契约，而且它们目前确实不一致。哪条是权威取决于你想验收什么。** ⟨07-20：契约已机器化（P1.4 六断言+D3 绊线）；分歧本身升级为 D3 待决——见文首状态表⟩
