...
- **硬编码 Windows 绝对路径**：`AIR_XLSX_DEFAULT = D:\Postgraduate\server-pyfluent\Air\Cfd-air-raw-old-new.xlsx`（`sjtu_tpmshx/df_surrogate/smooth_df.py:55-56`）。仅 SmoothDF 重建 CLI 用到；Linux 上重建需 `--air-xlsx` 传入。日常推断不触发。
- **中文路径/sheet 名**：训练 Excel 文件名与 sheet 名含中文（`试验记录表_整理版.xlsx`、`Gyroid_汇总`、`边界效应系数`，`sjtu_tpmshx/df_surrogate/surrogate_v3.py:85,173,179`）。Linux 上需确保文件系统与 locale 为 UTF-8。日志行刻意保持 ASCII——Windows GBK 控制台下中文日志曾毒化 pytest capture 流（`surrogate_v3.py:151-155` 注释记载）；各 CLI 均有 `sys.stdout.reconfigure(encoding="utf-8")` 防御（`surrogate_v3.py:612-615` 等）。
...
- **金值测试平台锁定**：`test_df_backend_registry.py` 的精确浮点金值是 Windows 开发机捕获的，跨平台 libm/FMA 差异约 1e-13，`CI=true` 时自动 skip（`sjtu_tpmshx/tests/test_df_backend_registry.py:30-35`）——Linux 服务器上跑该测试需设 `CI=true` 或接受最后 ULP 失败，勿以此判定移植失败。
