...
- **`E:\LWH\vault\reports\_research-ledger-CN.md`** — 调研台账（单一真源）：证伪方案（附推翻条件）+ 搁置与已知缺陷（附重启触发）+ 想法池 + 能力（活维护）。**任何闭合标定 / 数值格式 / 物理增补类调查开工前先查它**，做完当场回写；回写结论须溯源可执行代码 file:line，注释与旧报告只作线索。数字溯源查 `sjtu_tpmshx/validation/_CSV_STATUS.md`。
...
- **Nu coefficients have a single source:** `solvers/nu_correlations.py` → `NU_COEFFS` (air) / `WATER_NU_COEFFS` (water) / `SCO2_NU_COEFFS` (sCO2, smooth-wall CFD 2026-07-23 refit — 20 Diamond + 17 Gyroid geometries L∈[4,8], both topologies, form has an extra `(D_h/L)^d` term). Never duplicate them elsewhere. sCO2 Δp/Nu are SMOOTH-WALL estimates (roughness deliberately unanchored until sCO2 experiment γ lands — ledger SCO2-CFD); the sCO2 cF lives in `df_surrogate/sco2_df.py` and enters via `predict.sco2_cf_scale`.
