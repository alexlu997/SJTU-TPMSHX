# simpler-coupling-2d Specification

## Purpose
2D 生产求解器（`solvers/simple_solver.py` `SIMPLESolver`）的 opt-in SIMPLER 压力-速度耦合模式（Patankar/Tao：伪速度压力方程直接解 P + 仅速度 p' 修正）。基准结论为**负结果**（openspec archive `simpler-coupling-2d`，`reports/simpler-coupling-2d/CONCLUSIONS.md`）：精确直接解 PP 的 SIMPLE 无压力外迭代瓶颈，SIMPLER 为纯开销（0.5-0.6×）。模式保留为 experimental，默认路径不变；本 spec 约束其行为与不变量。
...
- **THEN** ΔP 相对差 ≤1%，v/P 相对 L2 差 ≤1e-2，u 按 ‖v‖ 归一 ≤1e-2
