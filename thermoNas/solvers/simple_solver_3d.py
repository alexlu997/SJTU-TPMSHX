...
with PyAMG-based pressure-Poisson solution. Designed for the ThermoNAS 3D
...
Key design choices for this MVP (Phase 1):
  ...
  * PyAMG smoothed-aggregation for the pressure-Poisson solve; hierarchy
    rebuilt every `pyamg_rebuild_every` SIMPLE iterations (default 10) to
...
class SIMPLESolver3D:
    ...
    def __init__(self, Lx, Ly, Lz, Nx, Ny, Nz,
        ...
        # Mass-flux anchor: G_inlet = rho_inlet * v_inlet (conserved across rho update).
        # Also store normalized inlet pattern to preserve spatial shape across rescales.
        rho_in_avg = float(self.rho_field[:, 0, :].mean())
        v_in_avg = float(self.v_inlet_field.mean())
        self.G_inlet = rho_in_avg * v_in_avg
        if v_in_avg > 1e-30:
            self._v_inlet_pattern = np.ascontiguousarray(
                self.v_inlet_field / v_in_avg, dtype=np.float64)
        else:
            self._v_inlet_pattern = np.ones((Nx, Nz), dtype=np.float64)
    ...
    def _update_density(self):
        """Compressible rho update (mirror 2D SIMPLESolver._update_density).

        ρ = P_abs / (R·T), under-relaxed. Also rescales v_inlet_field so total
        mass flux G = ρ_in · v_in stays constant across the update.
        Skips for incompressible fluid_type.
        """
        ...
        # Rescale v_inlet_field absolute (not compounding)
        rho_in_avg = float(self.rho_field[:, 0, :].mean())
        if rho_in_avg > 0.01:
            v_mean_target = self.G_inlet / rho_in_avg
            # Use stored pattern so rescales are absolute, not compounding
            self.v_inlet_field = np.ascontiguousarray(
                self._v_inlet_pattern * v_mean_target, dtype=np.float64)
            self.v_inlet = float(v_mean_target)
    ...
    def solve(self, max_iter=3000, tol=1e-6,
        ...
        for it in range(1, max_iter + 1):
            _sweep_u_jit_df_3d(self.u, self.v, self.w, self.P, self.d_u,
                                Nx, Ny, Nz, dx, dy, dz,
            ...
            _sweep_v_jit_df_3d(self.u, self.v, self.w, self.P, self.d_v,
                                self.v_inlet_field,
                                Nx, Ny, Nz, dx, dy, dz,
                                self.rho_field, self._mu_eff_field, self.mu_field,
                                self.K_arr, self.cF_arr,
                                self.outlet_frac, self.inlet_frac,
                                self.alpha_u, n_inner)
            _sweep_w_jit_df_3d(self.u, self.v, self.w, self.P, self.d_w,
                                Nx, Ny, Nz, dx, dy, dz,
                                self.rho_field, self._mu_eff_field, self.mu_field,
                                self.K_arr, self.cF_arr,
                                self.outlet_frac, self.inlet_frac,
