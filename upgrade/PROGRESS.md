...
## iter 70 · 2026-07-22 · D-2b-2 γ_HX 气侧 ✅——**两拓扑一致 ×1.26，D 侧双层闭合**（`68b45fa`）⏸ 循环随后暂停

- 7-6 HX 气侧 ÷ 试件层可压缩闭式：γ_HX(D)=1.27 [1.22,1.32]、γ_HX(G)=1.25
  [1.16,1.32]，流量近平——**HX 系统层几何无关**（正是歧管/入口效应该有的样子）
- **双层合成账**：D 2.39×1.27=3.04 vs 需要 3.10 → 闭合（2%）；G 2.26 vs
  上海标定 2.92 → 残差 ×1.29（534.8 旧栈伪迹 vs 样机差异，D-2c 盲考裁决）
- 口径调和：A_flow=ε_side×迎风面积三方自洽；G 首版误用 D 值当轮抓纠
  （×1.094 → Forchheimer ×1.20——几乎翻转 G 侧结论的量级，教训：逐拓扑
  硬件常数绝不共享默认）
- 门：套件 1306+10 绿；golden 双位同
- **Alex 指令暂停循环**（"完成这步后请先暂停"）：定时器 8f180729 已拆，
  状态文件原地，恢复=新会话说"继续升级循环"或本会话直接指示

## iter 69 · 2026-07-22 · D-2b-1 γ_specimen 面 ✅——**Gyroid 统计级证明 HX 层必需**（`6e64e87`）

- 纯试件 γ 面（dev 基，L6/L8 锚，t 斜率池化贝叶斯，n=6/拓扑）：
  G 面极紧（σln 0.024，回代 ±2%）、D 面松（σln 0.139，本征粗糙一致）
- **判决性数字**：γ_specimen(G,7,0.6)=1.81 [95%: 1.62,2.03] vs HX 需要
  2.92 → **缺口 ×1.61 出 95% 带**——HX 层不可由试件粗糙度解释，双层架构
  统计必需；D 缺口 ×1.30 带内（不可判别，双层作正则化）
- 上海 16 零参与（HX 需要值只作缺口读数）；八个排队 tick §6 合并恢复
- 门：套件 1306+10 绿；golden 双位同
- 下一步：D-2b-2 γ_HX 标定（7-6 双流体 + 口径调和 + 跨流体一致性）

## iter 68 · 2026-07-22 · D-2b 前置复核 ✅——**L6 平反 + 试件/HX 双层 γ 架构提案**（`9379510`）

- 06-30"L6 尖峰异常"复核撤销：L6/L8 是全表最干净数据（R² 0.996-0.9997，
  证伪层 L4/L5 恰是最糟 0.90-0.94）、隔离因子无断点、两拓扑同构——
  重释为 γ(L) 陡坡（相对粗糙度物理）；L4/L5 维持弃用；D/G 在 L8 分歧记开放
- **核心发现**：试件级 vs HX 级在门几何差 ~1.6×（纯试件插值 γ(G7)≈1.9 vs
  上海/7-6 需要 ≈3.0）——与 sCO2 XFLUID ×1.7-4.6 跨流体域同构 ⇒ **HX 级
  系统效应是真实独立层**（歧管/入口/多通道分配）
- **D-2b 架构提案**（审计 §8）：γ_total = γ_specimen(L,t)〔试件 L6/L8〕×
  γ_HX〔仅 7-6 HX 双流体实验标定，上海 16 不进——真盲考不破〕；纯试件
  方案照跑作对照（盲考差值 = HX 层定量）；sCO2 γ_f 形成跨域对照
- 工具 anchor_health.py（三证据轴一条命令）入库；门：套件 1306+10 绿、
  golden 双位同
- 下一步：D-2b 实施 ①γ_specimen 面 + ④对照组先行

## iter 67 · 2026-07-22 · D-2a-2 LOO 考卷 ✅——**dev 基 Gyroid 面 1.5%/2.5%，D-2a 全收案**（`12cee5c`）

- 留一几何 log-TPS 三方对比：G 侧 dev 基 cF LOO 1.5%（core 3.2%，再砍半）
  K 2.5%；D 侧面本征粗糙（12.5% med，dev 尾部 27% vs core 44-60%——中位
  小增买尾部大减）；SmoothDF 去水平形状散差 G 4.0%/D 13.7%
- D 侧插值不确定度 ~15-25% 如实入 UQ 账（与 06-30 L7/L8 双 CFD 系统差
  1.3-2.4× 同源）
- Alex 轮中确认点：水 CFD 对比必须用未经 sCO2 修正的 DF——结构性保证
  （考卷纯 CFD 层 + 产线分支隔离 + sco2_df 红线段"验证直连光滑"）
- 门：套件 1306+10 绿；golden 双位同
- 下一步：D-2b 前置复核（col47 L4/L6 异常——锚层重划裁定）

## iter 66 · 2026-07-22 · D-2a-1 发展段重提取 ✅——**候选表落库 + 入口污染系数级定价 + df-coeffs-cfd-refit 考古**（`751bf22`）

- 水侧 dp 口径核查（承 iter 65）：整核/发展段(2+3) = D 1.015 / **G 1.155**
  （随 Re 升到 1.5，Forchheimer 支配）——sCO2 发现同构坐实
- 方法教训：单发 NNLS ≠ 06-30 两段法（K 偏 4-7×，红灯校准立功）；两段法
  字面实现 + 阈值自校准（Re_hi≥12800/Re_lo≤400）→ core 复现对生产表
  med 0.98 紧散布，相对结论免疫估计器系统项
- **系数级定价**：cF_dev/cF_core = D 0.949 / G **0.831**；K_dev/K_core =
  D **0.846** / G 0.950；SmoothDF(Re_ref)/cF_dev = ×1.53 两拓扑一致
  ⇒ 已入产线的 K 面按发展段口径待下修（水侧 Δp 波及，归 D-3 重基准）
- **考古**：openspec df-coeffs-cfd-refit 是活变更（未归档），2026-06-30/07-01
  已交付 LOO 骨架（cF LOO 5.1/14.5% vs gamma_df 面 87/122%，~6×优）、
  air↔water 流体无关证明（0.96）、**col47 锚 L4/L6 异常**（L6 是可信层！
  直接冲击 D-2b 设计，开工前须复核）；候选 D 水/空气阶段 = 其续篇
- 门：套件 1306+10 绿；golden 双位同（生产零改动）；工具实跑 exit 0
...
## iter 72 · 2026-07-23 · R2：D-2a 重提——dev 表切换修正 Um 基（`8190ccc`）

**做了什么**：extract_dev_coeffs.py 数据源从退役的 water-cfd-raw.xlsx 切到
load_water_cfd（修正上传）；两段法数学与切点冻结不动；阈值自校准步骤退役
（目标生产表是旧 Um 基）；D_7_3/4/5 带 flow_suspect 列入表。

**证据（三线交叉验证）**：①u 不变量（dev/core 比）与 iter 66 逐位一致——
入口污染结论跨修正成立；②40 几何位移与 (u_旧/u_新)² 定律逐位吻合
（D_7_6 +28.5%、G_7_6 −8.6%、全表中位 −8.3%）；③LOO：Diamond dev cF
medAPE 12.5%→**9.6%**（旧 Um 随机差修掉、面更规则），Gyroid 2.0%/2.2%
保持。两工具 venv 运行 exit 0（§4 工具行门）。

**下一步**：R3 = D-2b 重裁决（γ_specimen + γ_HX 在新 dev 表上重跑；
iter 69/70 的"缺口 ×1.61 出带""γ_HX ×1.26 拓扑无关""D 双层闭合 2%"
三个数字结论全部重验；audit §8/§9 勘误）。

## iter 73 · 2026-07-25 · R3：D-2b 重裁决 + 模型切换（`d7d9a1d`）

**做了什么**：①γ_specimen + γ_HX 气侧在 R2 的修正 dev 表上重跑，iter 69/70
三个数字结论逐一重验（audit §10，§8/§9 加勘误指针）；②按 Alex 指示把
PROTOCOL §10 主循环模型从 Fable 5 改为 Opus（署名模板同步）。

**三结论重验（这是本轮的实质）**：
1. **"γ_HX ×1.26 拓扑无关"被推翻**——修正后 D 1.08 vs G 1.23 明显分化。
   旧的表观一致是 Um 伪迹：D_7_6 的 cF 在旧 Um 下低估 28%，恰好把 D 侧
   γ_HX 顶到与 G 相同的 ~1.26。且 D/G 气侧实验来自不同台架/批次
   （D 20260609 水直空气侧 / G 20260407 上海系调换进出口），1.08 vs 1.23
   是拓扑差还是台架差现有数据不可分辨——标注开放。
2. **Diamond "双层闭合" 改写为 "试件面基本单独闭合"**：γ_spec(D)=2.15
   缺口仅 ×1.13、落在 95% 带内，HX 层缩水成 ~8% 修正。iter 69 的
   "HX 层统计必需"降级为 **Gyroid 独有**（G 缺口 ×1.59 仍出 95% 带）。
   附带铁证：γ_spec(D) 的 σln 0.139→**0.051**（塌缩近 3×）——旧 Um 的
   逐几何误差一直伪装成试件散差，这是修正数据权威性的第四条独立证据。
3. **G 对上海标定 534.8 的残差 ×1.296 量级确认**（原 ×1.29，data-over-data
   比值，CFD 基不变量）。两假说（534.8 旧栈伪迹 vs 样机真实差异）与 D-2c
   盲考裁决法原样成立；新增侧证：D 侧近无 HX 残差 ⇒ 若 G 的 ×1.29 是真物理，
   必为 G 样机/上海台架特有，而非通用歧管效应。

**模型切换**：Alex 原话"用 opus 5、暂不用 fable"。核实——`/model` picker
最新 Opus 为 4.8，无 Opus 5 型号；loop 里也无 "opus4.8" 字面设定（§10 主循环
本是 Fable，委托用 `model:"opus"` 家族名路由最新 Opus）。按意图落地：PROTOCOL
§10 主循环改 Opus（最新 4.8），暂停 Fable；cron 新会话按 Alex /model 默认
（已设 Opus 4.8）自动跑 Opus。本轮 R3 由 Fable 跑完、如实署名 Fable。

**证据**：两工具 venv 重跑 exit 0；无代码改动（生产零涉，golden/suite 不需）。
定时器按 §8 重建（job 649f1bdf，`7,22,37,52`，armed 2026-07-25）。

**下一步**：D-2b-3——水侧 γ_HX（修正 dev cF 基）+ 跨流体一致性
（空气 D 1.08/G 1.23 vs 水 vs sCO2 γ_f）+ 双层合成面 UQ + D-2c 盲考准备。

## iter 74 · 2026-07-25 · D-2b-3：γ_HX 水侧 + 气/水跨流体对照（`7ea25ec`）

**做了什么**：新增 `gamma_hx_water.py`，在 7-6 样机**水侧**压损实验
（D_7_6 18 工况 / G_7_6 16 工况）上提 γ_HX，并与气侧同口径对照。选水侧
先做是因为它比气侧干净三点：试件层的基 cF_dev 本身就是水 CFD 提的
（不含"cF 跨流体不变"这个前提）、不可压 Δp 直读、**两拓扑同工作簿同批次**
（正好消掉 iter 73 那个"拓扑差 vs 台架差不可分辨"的混杂）。

**两条结论（审计 §11）**：

1. **γ_HX 不是只依赖拓扑的稳定几何因子。** 水侧 D 2.44 / G 2.18（G/D 0.89），
   气侧 D 1.08 / G 1.23（G/D 1.15）——**序反号**。iter 73 把气侧那对数标为
   "拓扑差还是台架差不可分辨"；水侧消掉台架维度后给出相反的序，所以
   "分化 = 拓扑属性"失去支撑，这两个数不能当拓扑参数写进合成面。

2. **水侧 γ_HX 约为气侧两倍，最省的解释是几何口径不是流体。** 气侧工具按
   "双流对称芯两侧同几何"把整机 A_flow 推广给水侧，但同一份 docstring 早就
   记着表内 样机流量/单个流量 ≈ **34（气）/28（水）**——两侧通道数不同。
   反解使两侧对齐所需的水侧面积 = D 0.626× / G 0.733× A_air（等效通道比
   1.60/1.36 vs 标称 1.21）：方向量级都对上，但朴素"面积 ∝ 通道数"只把水侧
   压到 1.76/1.53，仍差 ×1.63/×1.24，收不了口 ⇒ **DECISIONS D8**，
   水侧闭合挂起待数据方给流通面积。

**顺手挖出的两处原始表缺陷**（标记入 CSV，不静默丢弃）：G_7_6 工况1
Δp = **−48.4 Pa**（负压差，传感器地板）；D_7_6 工况10/11 除 ṁ 外**逐位相同**
（复制粘贴痕迹，其一必错，两条同退）。

**两个自己差点踩的坑（留痕在工具里）**：①Darcy/Forchheimer 配比 =
Dh/(Re·K·γ_spec·cF)，**只是 Re 的函数**——水/气 Darcy 份额差
（0.30/0.21 vs 0.04/0.03）反映的是工况点在 Re 轴上的分布位置，不是"Re 窗不
重叠"（两窗实际重叠 42–1102 / 438–8235，首版判读写错已纠）。②重叠带里的
直接对照太薄不能用：气侧只有 n=2 / n=1 落在带内，且正是气侧自身最低流量端
（γ_HX 低至 0.33，<1 意味着测值低于试件层预测 ⇒ 气侧也有低流量地板）——
工具打 `[薄]` 标并声明不作裁决用。

**证据**：venv 实跑 exit 0（§4 独立工具行），ruff All checks passed；
生产零改动（新脚本无包内导入者），套件/golden/上海门不涉。

**下一步**：D-2b-4 双层合成面 + UQ——**水侧腿 BLOCKED on D8**，气侧腿可做，
但写法要改：γ_HX 按拓扑常数固化已被本轮证否，合成面需改为"气侧标定 +
带宽吃掉拓扑不稳定性"。之后 D-2b-5（sCO2 腿）→ D-2c 上海盲考。

## iter 75 · 2026-07-25 · D-2b-4：双层合成面 + UQ 带（气侧腿，`3981f0f`）

**做了什么**：新增 `gamma_two_layer.py`，把两层合成成一张可外推的面
γ_total(topo,L,t) = γ_spec(L,t) × γ_HX(topo)，并给出逐案 Δp 预测带与
γ_total 面的外推带。这是 D-2b 系列的收口件。

**本轮最重要的一件事是"发现自己的带是假的"**：做合成时 Gyroid 的 γ_HX 层
σln 算出来 0.339，可 iter 73 报的 [P10,P90]=[1.15,1.30] 只对应 σln≈0.048
——两者不可能同时成立。逐案排查，气侧两表**也**各有一个仪表地板案
（D 工况1 Δp=893Pa γ=**0.69**、G 工况1 Δp=336Pa γ=**0.33**，其余各案
都在 1.02–1.34），而且 **D 表 工况10/11 是重复行**——与上一轮在水表 D_7_6
查到的 工况10/11 **同址同型**，两表同源复制粘贴，互相坐实。

修正的影响很干净：**中位数几乎不动**（D 1.08→1.08 / G 1.23→1.24），
**动的是散差**——σln D 0.126→**0.032**、G 0.339→**0.053**。也就是说
iter 70/73 的点估计站得住，但此前的**带**是被地板案撑出来的虚胖。
（虚胖的带也是错的：它让 68% 名义带实测覆盖到 89–94%。）

**UQ 的结构改了，这是与 iter 69/70 写法的实质差别**：γ_HX 是拿 γ_spec 的
点估计除出来的，两层**强反相关**（γ_spec 大一分，标出的 γ_HX 就小一分），
所以把两层的带各自独立抽样再相乘，等于把 γ_spec 的绝对水平不确定度重复
计入一次。正确的分解是：**HX 实测直接钉住标定几何 (7,0.6) 上的 γ_total**
（带 = 案间散差），**γ_spec 层只负责形状**（怎么外推到别的 L,t，带 = 参数
不确定度）。在 (7,0.6) 处形状项恒为 0，带只剩 HX 散差——那儿本来就是直接
测的；外推越远形状项越宽，把外推的代价显式摆出来。

**裁决（LOO，样本外）**：per_topo vs pooled 两变体并排跑，
medAPE 3.5% vs 5.9%、95% 带宽比 1.22 vs 1.49、68% 覆盖 70% vs 73%
（名义 68%）——per_topo 精度和带宽双赢，**气侧内部用 per_topo**
（γ_HX D 1.078 / G 1.228）。但必须钉死适用面：这条只对气侧成立，
拓扑分化的不可迁移性是**跨流体**的（iter 74 的 G/D 序反号），
本面的带是"气侧内"带，不得对外宣称覆盖水/sCO2。

**证据**：df_refit 全部 7 个工具 venv 实跑 exit 0；ruff 全目录 All checks
passed；`_prebuilt/` 生产表零改动，生产路径零涉；审计 §12 落账，§11 加勘误
（气侧 Re 窗抬高后 Gyroid 与水侧不再重叠，两条主结论与反解面积不受影响）。

**另修**（上一 commit `36fed34` + 本轮）：df_refit 6 个工具里有 4 个在本机
GBK 控制台跑到记分板 print 就崩（"⇒"/"∓"/"R²" 不在 GBK 里），CSV 在崩之前
已写出所以看着像成功、实际 exit 1 —— §4「独立工具行」要的正是"实跑 exit 0"，
此前若干轮的该项证据因此站不住。现 7 个全绿。

**下一步**：D-2b-5（sCO2 腿并入：γ_f hot 7.0-7.9 vs 双层合成；sCO2 f 侧
重提 K 面 + sco2_df cF 在新导出 + γ_f 重冻）→ D-2c 上海盲考。
D-2b-4w（水侧腿）仍 BLOCKED on D8。

## iter 76 · 2026-07-25 · D-2b-5a：三流体 cF 反演对照（`983822a`）

**前置**：STATE 让我开工先查 sCO2 表有没有气/水表那两类缺陷。查了：**没有
重复行**（两拓扑两侧全查）；冷侧确实压在仪表地板上（ΔP 低到 −0.027 MPa，
最低正 ΔP 处 f 散到 0.28–3.54），但 loader 的 `ok_dp` 早就挡住了、而且 D6
当初选的就是热侧；**热侧干净**（最低 ΔP 0.012–0.014 MPa 远离零）。
结论是好消息：γ_f hot 的标定不因此需要重做。

**正题，也是本轮真正的收获**：γ_f 和 γ_total **挂在不同的光滑基上**
（sCO2 CFD vs 水 CFD 发展段）。`sco2_gamma_f` 的 docstring 里那句沿用已久的
"γ_f 超额 γ_air ×4.0–4.6"，因此把"两个基不一样"和"两套数据打架"混在了一起——
从没分开算过。本轮把闭合层整个剥掉，只借一个 K，直接从三种流体的原始测量
反演同一个物理量 cF：

| | 空气 | 水 | sCO2 | 水/气 | sCO2/气 |
|---|---|---|---|---|---|
| Diamond | 429.8 | 1339.5 | 1522.1 | ×3.12 | **×3.54** |
| Gyroid | 400.5 | 829.3 | 1511.6 | ×2.07 | **×3.77** |

cF 是几何量，同一个芯本应同值。序还很稳定：气 < 水 < sCO2。

**这一轮最有价值的一句是"排除了什么"**。sCO2 这条腿**不享受** D8 那个逃生口：
气侧和 sCO2 用的是**同一个** A_flow（sCO2 表自带 5.9359e-4/6.4915e-4，与气侧
常量差 0.07%/0.13%），而 cF ∝ A² —— **A 即使整体错了，也在 sCO2/气 的比值里
精确约掉**。同样被排除的还有长度口径（同 L）、Re 外推（两窗擦肩，近匹配对照
×3.77/×3.85 与全窗中位一致，且各自窗内近平）、光滑基不匹配（本工具压根不用
cF 闭合；顺带量化：两基在相关 Re 只差 1.03–1.17×，本来也不够）、以及 K 的
选择（气/sCO2 对 K±50% 近乎免疫）。

**自洽核**：气侧反演出的 429.8/400.5 与上一轮双层合成面的 γ_total×cF_dev
（434.3/413.2）吻合到 1–3% —— 一条完全不用闭合的路径复现了合成面，两边互证。

**去向**：登记 **DECISIONS D9**，问数据方三件事（sCO2 压差测点是否含集管、
ṁ 是整机还是单侧、两次实验是否同一件芯）。收口只需要**单一量级**的修正
（抹平 D 的 ×3.54 只需 sCO2 Δp 实际低 72%、或 ṁ 实际高 88%），不是需要新物理
的量级。在答复前：γ_f 冻结常数**照旧不动**（它在自己口径内自洽、LOO 表现好，
是可用的工程修正），但不得被解释成表面粗糙度物理——docstring 已就地更正；
**跨流体统一 γ 面的设想搁置**。

**证据**：df_refit 全部 8 个工具 venv 实跑 exit 0；ruff 全目录 All checks
passed；`_prebuilt/` 与求解器路径零改动。动了一个生产文件但只是 docstring，
另跑 targeted `test_sco2_gamma_f` + `test_sco2_gamma_nu` 21 passed 佐证。

**另**：PROTOCOL §2 记入 GBK 控制台坑——诊断脚本里被 print 的字面量含
`⇒ ∓ ² ṁ` 会崩在记分板，而 CSV 已在崩之前写出，**看着像成功实际 exit 1**。
iter 75/76 各踩一次、共耗三次重跑，写进环境备忘止损。

**下一步**：**D-2c 上海盲考**——只用气侧/上海口径，D8/D9 都不影响它，
候选 D 边界②"上海 16 例退出标定转纯盲考卷"在此兑现。可并行的非阻塞项是
D-2b-5c（sCO2 f 侧重提，sCO2 口径内部的事）。

## iter 77 · 2026-07-25 · D-2c：上海 16 例真盲考（`4adaddc`）

**做了什么**：兑现候选 D 边界②"上海 16 例退出标定转纯盲考卷"。生产
`gamma_df` 的 Gyroid L 方向是**过 (L6, L7=上海标定 534.8, L8) 的对数二次**
——上海标定就烘焙在生产 γ 面里，所以在生产闭合下跑上海门是**自己考自己**。
本轮把整张面换成 D-2b 链条的双层合成面（链上零上海数字），真盲预测：

| | RMSRE_dP | 偏置 | 偏低例数 | RMSRE_Q |
|---|---|---|---|---|
| production（含上海标定） | 4.88% | +0.76% | 8/16 | 2.11% |
| **two_layer 盲考** | **25.29%** | **−24.69%** | **16/16** | 2.10% |

控制组逐位复现 BASELINE。逐例看，除最低 Δp 的工况1 外，工况2–16 全部落在
−22%…−28%，平得出奇；而 **Q 几乎不动** —— 纯阻力效应，不是热侧。
按 iter 73 立的判据，这是假说 **(ii)** 的教科书签名。

**但"真实差异"不该停在玄学，所以我去翻了原始表** —— 这是本轮真正的收获：

- 考卷 = `20260401-上海电气天然气加热器实验工况.xlsx`
- γ_HX 锚 = `20260407-上海电气天然气加热器实验工况 -**调换进出口**-G_7_6.xlsx`

**两本是同一台样机、同一组 16 个工况（样机流量逐位相同）、相隔一周、
进出口调换。** 逐工况实测 Δp 之比（0401/0407）= **中位 1.274**，区间
[1.206,1.352]。而盲考隐含的因子是 1.328、iter 73 记的 G 残差是 ×1.296
—— **三个数同源**。

⇒ **γ_HX 依赖流向。** 0407（调换后）是低方向、0401（原接法）是高方向；
生产的 534.8 对应 0401，双层合成面锚在 0407。**两者各自都对，描述的是
不同的管路接法** —— 歧管不对称造成的方向依赖，本来就是 HX 级系统效应的
正常表现。一个悬了四轮的"伪迹还是真差异"，落地成了一个具体、可核实的事实。

**这带来一个产品问题，不是数据问题** ⇒ **DECISIONS D10**：产品交付到底按哪种
接法？选错方向就是系统性 27% 的 Δp 偏差。在 Alex 答复前生产默认一律不动
（534.8 对应 0401，而上海门是当前唯一的端到端认证门），**D-3 换默认 BLOCKED**
——双层面原样换默认会把上海门从 4.88% 打到 25.3%。

**顺带两件**：①iter 74 那个"气侧 G/D 序反号"要再添一个混杂项——G 的锚是
**低方向**测量，折算到高方向 G/D 从 1.15 变 ~1.45；§11 结论 1（γ_HX 不是纯
拓扑常数）不变，但"归台架"应读作"归台架**或流向**"。②0407 的工况1
（Δp=336 Pa）正是 iter 75 判为仪表地板的那一案，0401 同工况读 1149 Pa
—— 反过来印证了那次筛。

**证据**：本工具 venv 实跑 exit 0（内含两次完整上海 3D 门）；df_refit 全部
9 个工具 exit 0；ruff 全目录 All checks passed。生产零改动——`two_layer`
backend 只在本脚本进程内注册，默认 `TPMSHX_DF_METHOD` 不动。

**下一步**：待决已堆到三条（D8/D9/D10）。当前**唯一不被阻塞**的主线是
**D-2b-5c sCO2 f 侧重提**（K 面 + sco2_df cF 在新导出 + γ_f 重冻，
sCO2 口径内部的事）。若三条久悬，循环可做的就只剩它与文档/质量类条目了。

## iter 78 · 2026-07-25 · D-2b-5c：sCO2 f 侧重提 + γ_f 重冻（`16a10ef`，§5 重基准）

**做了什么**：R1（iter 71）把 2026-07-23 的修正 CFD 上传同步进了 `data/`，
但只重拟了 Nu 侧——f 侧的 `_prebuilt/sco2_df_coeffs.csv` 停在 07-19，滞后了
两轮。本轮补上，并按 PROTOCOL §5 走完整的重基准流程。

**基重建**：几何 **D 15→20 / G 12→17**，例数 **7000→9799**；B 中位 −10.6%
（区间 −25.9%…+35.8%），pooled m 两拓扑各降约 5%；K/ε_f/Dh 不变。

**γ_f 重冻**：`γ_f ≡ f_exp/f_cfd` 是 base-relative 的，所以基一动常数必须
跟着动——`test_sco2_gamma_f.py` 的 base-swap 绊线正为此而设，本轮按设计
炸响（两拓扑 FAILED）。**重导常数，没有放宽容差。**

**这次重基准的风险很低，而且有个漂亮的自洽**：dexp 的降幅与 pooled m 的
降幅几乎精确抵消（D −0.00850 vs −0.00868）——因为实验数据没变，
`γ_f × cF_smooth` 这个乘积就不该变。实测生产真正消费的
`γ_f(Re)·cF_smooth(7,0.6,Re)` **全窗变化 ≤0.02%**。真正的变化落在**其它
几何**（Re=2e4 处光滑基 −8…−13%），那是 10 个新几何、2800 个新例带来的
信息增益，不是漂移。

**一个实质改善**：新导出把网格补满，**(7,0.6)——也就是 D-7-6/G-7-6 样机
几何——从"域外 RBF 外推 + 告警"变成了插值节点**（旧域 Diamond L∈[4,7] 且
无 7/0.6、Gyroid 只到 L=6）。样机几何的 sCO2 闭合不再骑在外推上。

**全 V&V（§5b）**：套件双 pass **1306 passed / 3 skipped + 10 串行**；
golden `--check` 裸调用 **PASS (bit-identical)**；上海 3D 门 **16/16 valid、
dP 4.88% / Q 2.11%、GATE PASS**（与 BASELINE 逐位一致）；lumped **Q_air 1.76%**
持平 —— 空气/水认证面零影响，符合 sCO2 路径隔离的预期。
D11 登记 [已重基准-待复核]；台账 **SCO2-F-REFIT-0725** 已回写（§5e）。

**副产物：我自己造了一个假 FAIL，值得记下来。** golden 第一次报 FAIL、
39 条 ULP 级 diff，我差点当成回归。三步对照实验：①干净 HEAD 跑出**逐行
完全一致**的 39 条 diff ⇒ 我的改动贡献恰好为零；②两次独立运行彼此位同
⇒ 求解器确定性；③**裸调用**干净 HEAD **PASS (bit-identical)**。根因是我把
`run_tests_server.ps1` 的五个线程钉套到了金门上，改变了 AMG/BLAS 的归约
顺序——而 PROTOCOL §4 和 BASELINE 记的本来就是裸调用。已写进 PROTOCOL §2，
并附一条推论：**金门 FAIL 时先用干净 HEAD 跑同一条命令做对照，再谈回归**。
这跟 iter 75/76 那个 GBK 坑同类，都是口径混用造成的假阳性。

**流程自查**：本轮开工时我没有先把目标写进 STATE 的 `in_progress`（§1.2
要求），中途被 cron 触发时磁盘上看不出有轮次在飞。发现后当场补写并附了
中断恢复指引，收轮时清空。

**下一步——需要 Alex 介入**：待决已堆到四条（D8 水侧 A_flow / D9 sCO2 压差
测点与 ṁ 口径 / D10 交付接法 / D11 新几何 CFD 质量），全都要数据方或 Alex
答复；候选 D 的 D-2b/D-2c 主体已完成，D-3 换默认 BLOCKED on D10。
**iter 78 是最后一条不被阻塞的实质条目。** 下一轮起循环应先报告待决堆积
并请示优先级，等待期间只做文档/质量类条目，不硬推被阻塞的标定工作。

## iter 79 · 2026-07-25 · D-0：col47 试件锚溯源审计（`0dd90af`）

**为什么这轮做它**：候选 D 主线全被 D8-D11 卡住，而 D-0 是章程里立得最早、
却一直没做的一条——审的正是整条 γ 链的地基（γ_spec 全靠 col47 试件锚）。
它不依赖任何待决答复，所以是等待期最该做的事。

**把提取式钉到了行号**（`df_surrogate/surrogate_v3.py`）：

- **闭式反演，不穿求解器**（`:244-249`）——lhs=(P_in²−P_ATM²)/(2RT·L_ch) 对
  [μG, G²] 做 WLS，全程只有 `np.linalg.lstsq`，没有 SIMPLE、没有网格。
- **水平口径入锚**（`:246`）——`P_in = P_ATM + ΔP`，**出口恒钉 1 atm**。
  入锚样本的 ΔP/P_atm 中位 0.156–0.210、**最大 0.815**，所以这个假定是承重的。
  这正是 C8/打靶 里记的"γ_df 锚点吸收了旧口径压力水平偏置"的机理，现在有行号了。

**顺手做了缺陷筛**（前四轮在每张表都查出问题的那一套）：试件台账两拓扑
**非正 ΔP 0 行、重复行 0 行** —— **目前唯一一张两类缺陷都为零的表**。
这是对 γ_spec 的一条正面证据。

**实质发现：γ_specimen 不是纯粗糙度因子。** `:250-251` 对每个几何施加一个
α（工作簿 "边界效应系数" sheet）：`c_F = α·cF_raw`、`K = K_raw/α`。
α ∈ [0.374, 0.608]，是个 ~0.5 的系统性乘子，且随 t 单调降。把
`ln γ_spec = ln α + ln(cF_raw/cF_dev)` 拆开：
...
Gyroid 那行尤其值得看：γ_spec 在 t 方向近乎水平，**不是因为没有 t 依赖，
而是 α 的 −0.720 几乎正好抵掉了原始比值的 +0.720**；L6 锚的 t 走向也因此变号。
去掉 α，锚 γ 中位从 2.82→5.18（D）、2.14→4.37（G）——"粗糙度因子 ~2"
是净掉一次减半之后的数。

**影响分得很清楚，不夸大**：
- **水平无害**。HX 层按 γ_HX=Δp_meas/Δp_pred(γ_spec) 标定，γ_spec 的水平偏差
  被整体吸收。所以 §12 的 γ_total 水平、§13 的三流体 cF 反演、§14 的上海盲考
  结论**全部不受影响**——它们要么在固定几何上，要么是 data-vs-data。
- **形状有害**。γ_spec 的 (L,t) 外推靠的正是形状，而形状主要由 α 驱动。
  ⇒ **§12.2 的 shape-contrast 带只含参数不确定度，不覆盖 α 的出处风险**。
  在 (7,0.6) 处该带恒为 0（那里直接可测）故无碍；但**离开 7/0.6 做设计/寻优
  外推时，真实不确定度大于 §12.4 报的宽比**。D-3 换默认和任何几何寻优都得知道。
- α 的出处 = 工作簿一张 sheet，**仓库内无推导记录** ⇒ **DECISIONS D12**。

**证据**：venv 实跑 exit 0；ruff 全目录 All checks passed；生产零改动。

**下一步——建议转按需模式**：D-0 收了之后，**已经没有不被阻塞的实质条目**。
待决五条（D8 水侧 A_flow / D9 sCO2 压差测点与 ṁ 口径 / D10 交付接法 /
D11 新几何 CFD 质量 / D12 α 的推导）全部需要数据方或 Alex 答复。
再自动跑下去只会产出没有依据的数字。按 ROADMAP 末条转按需模式：
拆定时器、改由 Alex 手动触发，PROTOCOL 与状态文件原地保留可随时复活。

## iter 80 · 2026-07-25 · 记账校正 + 决策简报（无实质条目可做）

**处境**：D-0 收案后已无不被阻塞的实质条目。PROTOCOL §1.6 要求"绝不空转
等待"，但候选池里的 A/B/C 三条**循环无权自行开工**——Phase 5 头注写明
"收尾时 Alex 挑选"，而 Alex 选的是 D。所以本轮做两件确实在授权内、
且确实有用的事。

**①ROADMAP 记账校正**（查出三处滞后）：
- **D-2sc** sCO2 γ_f 修正试点——iter 61 就用 `gamma_f_variants.py` 六变体
  做完了，D6 也已裁定 hot-free 且常数早已入产线，却一直挂着未勾选。补勾。
- **D-2a** cF 光滑基升级——iter 72（R2，`8190ccc`）已把 dev 表切到修正 Um 基，
  LOO 反而更优。补勾。
- **D-2b** 父条改标 `[~]`：1/2/3/4/5a/5c 全收，余 4w、5b 两条 BLOCKED。

这类滞后本身值得警惕——它让 ROADMAP 看起来比实际剩得多，也让"下一条做什么"
的判断失真。

**②给 Alex 的一页决策简报**（`upgrade/DECISION-BRIEF-2026-07-25.md`）：
`DECISIONS-NEEDED.md` 现在有 12 条、每条都带完整背景，那是**参考文档不是
决策工具**——要在里面找出"我现在该拍什么"成本很高。简报补这个缺口：
五条待决**按杠杆排序**，每条只写三样东西——你要拍什么、决定它的那**一个**数、
拍了解锁什么。

排序与理由：
1. **D10 交付接法**（★）——实测 Δp 比 1.274 是同机同工况调换进出口测出来的；
   这是**产品事实不需要新实验**，且直接解锁 D-3 换默认。
2. **D8 水侧流通面积**——只要一个数（工作簿"流通截面积"列的水侧值）。
3. **D9 sCO2 压差测点/ṁ 口径**——比 D8 硬，因为面积口径已被 A 不变性排除；
   收口只需单一量级修正，不需要新物理。
4. **D12 α 的推导**——不影响任何已完成结论，但阻塞"把双层面用于 7/0.6 以外几何"。
5. **D11 新几何 CFD 质量**——确认题不是决策题，不答也不影响什么。

简报末尾还写了"如果一条都不答"该怎么办，以及候选 D 22 轮的产出盘点
（9 个工具、审计八节、**推翻过 3 个自己先前的结论**），供判断值不值得继续投。

**证据**：docs-only 轮次（§4 第四行免测）；无代码改动，生产零涉。

**下一步**：再次建议转按需模式。循环不擅自拆自己的触发器（§8 只授权
"无任务或过期时重建"），定时器 `5e52a278` 仍在，停跑由 Alex 决定。

## iter 81 · 2026-07-25 · D-2c'：闭合方法对比矩阵——534.8 基本可被预测出来（`66b23dc`）

**先纠正我自己上一轮的判断**：iter 80 我把 D-2c' 整条当成被 D10 阻塞、宣布
"已无实质条目"。那是**过度保守**——被 D10 阻塞的只是**裁决**，**打分台**完全
可以先建；而且把两种流向都算出来，反而把一个待决问题变成了一张预先算好的表。

**记分（上海 3D pipeline，20×10×3，16 例）**

| 变体 | cF(G,7,0.6) | RMSRE_dP | 偏置 | max\|e\| |
|---|---|---|---|---|
| production（含上海标定） | 534.80 | 4.88% | +0.76% | 16.01% |
| two_layer（零上海输入，锚 0407） | 413.29 | 25.29% | −24.69% | 28.38% |
| **two_layer_dir（×1.274 → 0401）** | **526.54** | **4.92%** | −1.06% | **14.92%** |
| smooth_only（γ≡1，下界） | 167.34 | 67.49% | −67.23% | 71.63% |

**核心结论**：一条**零上海标定**的闭合链——试件台架（col47）+ 7-6 样机气侧
台架 + 水 CFD 发展段基——加一个流向因子，与含标定的生产面**差 0.04 个百分点**，
cF 526.54 vs 534.80（−1.5%），max|e| 反而更好。
**534.8 从"标定出来的"变成基本"预测出来的"。** 这是候选 D 二十多轮里最硬的
一条正面结论——之前的收获多半是推翻自己（"×1.26 拓扑无关"、"D 双层闭合"、
"γ_HX 是纯拓扑常数"三条都被自己证否），这一条是正面的。

`smooth_only` 的 67.5% 则第一次定量回答了"双层架构值多少"：**约 13 倍误差改善**。

**诚实性边界我写死进了工具和审计**：`two_layer_dir` **不是纯盲考**——
DIR_FACTOR = 1.274 取自 0401/0407 实测 Δp 比，而 **0401 就是考卷本身**。
公平的说法是：production 用了 1 个上海标量（534.8），two_layer_dir 也用了
1 个（流向因子），**两者用掉的上海信息量相同**；差别在于 two_layer_dir 的
几何/粗糙度/HX 结构全部来自独立台架，而非拟合到考卷上。另外流向因子原则上是
**管路的物理属性**，在任一台机器上测过两种接法就能得到，不必依赖这份考卷。

**对 D10 的意义**：答 0401 → 读 `two_layer_dir`，它可直接进 D-3；答 0407 →
读 `two_layer`，同时意味着生产的 534.8 偏高约 27%、需按 §5 走一次重基准。
**无论哪种，双层架构本身已被这张表证明够用，差的只是锚在哪个方向。**

**证据**：venv 实跑 exit 0（内含四次完整上海门，数字三次运行逐位复现）；
ruff 全目录 All checks passed；生产零改动（各变体 backend 只在本脚本进程内注册）。

**下一步**：D-2c' 余项——物理化（Ergun 族）与贝叶斯标定两变体，各需一套独立
拟合，打分台已留接口。**这是不被阻塞的**：它们只是多两行记分，裁决仍等 D10。

**教训（iter 80→81）**：宣布"无事可做"之前先把条目拆开看——**被阻塞的常常
只是结论，不是全部工序**。

## iter 82 · 2026-07-25 · D-2c' 补建：物理化 Ergun 族（`46310e5`）

**做了什么**：把 Ergun 原式化到本仓的**间隙**流速约定，代入
d_p = 1.5(1−ε)D_h/ε 之后 **ε 解析约掉**，剩下

    cF = 1.75/(1.5·D_h) = 1.1667/D_h        K = 0.015·D_h²

也就是一个**零参数**闭合——不看实验、不看 CFD，只要几何。这正是"物理化"
变体该衡量的东西：纯经验关联式能走多远。

**五变体完整记分（上海 3D，16 例）**

| 变体 | cF(G,7,0.6) | RMSRE_dP |
|---|---|---|
| production（含上海标定） | 534.80 | 4.88% |
| two_layer_dir（零上海几何标定 + 流向因子） | 526.54 | 4.92% |
| two_layer（锚 0407） | 413.29 | 25.29% |
| **ergun（零参数）** | **339.27** | **41.12%** |
| smooth_only（γ≡1 下界） | 167.34 | 67.49% |

**一个反直觉的结果值得记**：`ergun`（41.1%）**优于** `smooth_only`（67.5%）。
不是因为 Ergun 比 CFD 准——而是因为光滑 CFD 基缺掉的粗糙度/HX 因子（~3.2×）
比 Ergun 相对光滑基的固有高估（339/167 ≈ 2.0×）**更大**。换句话说：
**在这个问题上，"用对量级的粗糙经验式"比"精确但缺了一层物理的 CFD"更接近
真值。** 这正是双层架构存在的理由，也是对"直接拿光滑 CFD 当闭合"这条捷径的
定量否决。

**口径我没有藏**：本仓 `tpms_geometry` 的 A_0 实测满足
A_0·D_h/(4·ε/2) = **1.000**，坐实 D_h 是**单侧**约定；而 Ergun 的 d_p 换算
标准式假定 D_h = 4ε/S_v。两种口径差 ~2 倍（cF 339.3 vs 169.6），而实测气侧
cF≈400 **正好被两者夹住**。这个 2 倍的约定自由度本身就是物理化路线的固有
不确定度——摆出来比挑一个报上去诚实。

**证据**：venv 实跑 exit 0；ruff 全目录 All checks passed；生产零改动。

**下一步**：D-2c' 最后一项 = 贝叶斯标定变体（接口已留）。做完之后就真的
只剩文档/质量类边角，而五条待决（D8–D12）仍全部需要人答复——届时**强烈建议
转按需模式**。

## iter 83 · 2026-07-25 · D-2c' 末项：后验带的诚实度检验（`fd97d7c`）

**这一轮问的问题和前三个变体不同**：前面比的都是点估计准不准，这个问的是
——**D-2b-4 给出的那条后验带，面对一份完全外部的数据集时诚不诚实？**
那条带建在试件台架 + 7-6 气侧台架上，**上海 16 从未参与它的构造**，
所以这是对该 UQ 声明最强的一次检验。

**方法上省了一大笔**：Δp 对 cF 单调，所以把 γ_total 的后验五分位送进求解器，
出来的**精确就是** Δp 的同阶分位——5 次解代替 MC 穿 3D 求解器的上万次。

| 方向 | 68% 覆盖（名义 68%） | 95% 覆盖（名义 95%） | 中位线 RMSRE |
|---|---|---|---|
| **dir（×1.274，0401 交付方向）** | **81%** | **94%** (15/16) | 4.92% |
| **raw（0407 锚方向）** | **0%** | **6%** | 25.29% |

**核心结论**：**同一条带，加了流向因子就是诚实的，不加则不只是有偏、
而是"自信地错"。** dir 方向下 95% 覆盖 94% 几乎精确、68% 覆盖 81% 略偏保守；
raw 方向下 68% 带覆盖 **0%**——16 个例子一个都没落进去。

**原因不难找，我在跑之前就写进了工具**：这条带只含 **HX 层的案间散差**
（标定几何处形状项恒为 0，§12.2），**不含流向这一项系统不确定度**，
也不含 §15 那条 α 出处风险。**统计不确定度量对了，系统不确定度整个缺席**
——所以一旦系统项没被外部因子吸收，带就完全失效。

**对 D-3 / 生产使用的直接含义**：闭合的带**必须带一个流向项，或者管路配置
必须被钉死（D10）**。只报 §12.4 那个 1.15–1.25 的宽比而不声明配置，
是在给出假自信。

**证据**：venv 实跑 exit 0（内含 10 次完整上海门）；ruff 全目录 All checks
passed；生产零改动。

**下一步**：**D-2c' 四变体全部建成，候选 D 的可做部分至此穷尽。**
剩余条目（D-2b-4w 水侧腿 / D-2b-5b γ_f 并入 / D-3 换默认 / D-2c' 裁决）
**全部在等 D8–D12 的人工答复**。强烈建议转按需模式，决策简报已备
（`upgrade/DECISION-BRIEF-2026-07-25.md`）。

## iter 84 · 2026-07-25 · 收尾：终审报告补章 + 转按需模式（定时器已拆）

**为什么是这一轮**：候选 D 的可做部分在上一轮穷尽（D-2c' 四变体全建成），
剩余条目——D-2b-4w 水侧腿 / D-2b-5b γ_f 并入 / D-3 换默认 / D-2c' 裁决——
**全部在等 D8–D12 的人工答复**。我连续三轮建议转按需模式，无回应。

重读 ROADMAP 时确认：**唯一一条未勾选、未 BLOCKED、且循环有授权执行的条目，
正是末条**——"都不挑 → 循环转按需模式：拆定时器，Alex 手动触发，
PROTOCOL/状态文件原地保留可随时复活"。按 §1.2 取第一个可做条目，就是它。
所以本轮不是"擅自停跑"，而是**执行章程里为这个局面预留的那一条**。

**做了两件事**：

1. **终审报告补候选 D 章**（`upgrade/FINAL-REPORT.md`）——原报告写于 iter 40、
   只覆盖 P0–P4。补章记下 iter 58–84 这 27 轮：三条最硬的结论
   （534.8 基本可被预测出来 / 上海残差的出身是流向 / 不确定度声明必须带流向项，
   否则是假自信）、**三条被自己推翻的旧结论**、五张实验表查了四张有病
   （试件台账是唯一干净的）、以及交付物清单。

2. **拆定时器**（job `5e52a278` 已 CronDelete），ROADMAP 末条勾选。

**为什么该停**：再跑下去只能做文档边角，而每一轮都消耗 Alex 的额度。
候选 D 已经把能在没有新信息的情况下问出来的问题都问完了——剩下的五条
**不是算力问题，是信息问题**，只有人能答。

**复活成本接近零**：在 worktree 目录说一句"继续升级循环"即可。
PROTOCOL.md / STATE.md / ROADMAP.md / DECISIONS-NEEDED.md 全部原地保留，
STATE 里写了复活后第一件事该查什么。

**给 Alex 的入口**（按省事程度排）：
- 一页决策简报：`upgrade/DECISION-BRIEF-2026-07-25.md`（五条待决按杠杆排序）
- 终审报告：`upgrade/FINAL-REPORT.md`（含候选 D 补章）
