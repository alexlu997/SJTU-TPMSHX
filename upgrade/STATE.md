...
- iteration: 69 ——**D-2b-1 收案（`6e64e87`）：γ_specimen 面落库；Gyroid
  缺口 ×1.61 出 95% 带 = HX 层统计必需（D ×1.30 带内不可判别）——审计 §8
  双层架构由证据坐实**（八个排队 tick 按 §6 合并为本恢复轮）
- next: D-2b-2 γ_HX 层标定——7-6 HX 双流体实验（换热器压损 G-7-6/D_7_6
  气侧 + 7-6-Water-dp 水侧）：①任务 3.1 口径调和（HX 级 Δp 的 core vs
  total/歧管/通道数约定逐表核对）②γ_HX 提取（对 γ_specimen×cF_dev 基）
  ③跨流体一致性检验（同一 γ_HX 须同时容纳气/水——06-30 设计约束）
  ④与 sCO2 γ_f 跨域对照。产出 = γ_HX 后验 + 双层合成面 + 上海盲考准备
