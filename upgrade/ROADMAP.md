...
- [ ] 候选 D · D-F 系数获取方法调查（Alex 2026-07-20 提出，研究/标定级）：系统调查是否存在
      比现行 gamma_df（cF = 光滑 CFD × 实验锚 γ 插值，K = CFD-refit log-TPS 面）更优的
      (K, cF) 获取路线——候选方向举例：扩充 CFD 训练点/更优插值形式、物理化模型
      （Ergun 族/孔隙结构参数化）、贝叶斯标定（不确定度量化）、多保真融合（CFD+实验分层）、
      逐工况局部标定 vs 全局面。**硬约束烘入**：①任何新 backend 换默认前必须复现 Shanghai 3D
      基线（CLAUDE.md 硬不变量，历史候选曾差一个量级）；②DF 已含 SLM 粗糙度，绝不引入
      双计乘子；③开工先查台账（rbf D7 外推证伪史、gamma_df 锚定沿革、SCO2-CFD 光滑壁
      待锚条目），结论回写台账；④与候选 A2 有耦合——D3 若改 G 口径则 γ 锚点移动，
      两项调查需排序或合并。产出 = 方法对比证据 + 台账"证伪方案"条目 + 换默认提案
      （最终 Alex 拍板，走 `!` 重基准流程）
...
- [ ] Phase 5 候选池评审：三池选单（附各自工作量与风险注记）交 Alex 挑选或转按需模式
