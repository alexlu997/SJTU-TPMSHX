...
- [ ] 候选 D · D-F 系数获取方法（Alex 2026-07-20 提出；**2026-07-22 立项对话拍板边界，
  ...
  - [ ] D-2b γ 重锚（水/空气）：纯试件锚（无上海点），Gyroid L 方向照
    ...
    - [ ] D-2b-4 双层合成面（γ_spec × γ_HX，分拓扑）+ UQ 后验带 —— **水侧腿
          BLOCKED on D8**；气侧腿可做（γ_HX 按拓扑常数固化的写法已被 D-2b-3
          证否，合成面需改为"气侧标定 + 带宽吃掉拓扑不稳定性"）
    - [ ] D-2b-5 sCO2 腿并入（γ_f hot 7.0-7.9 vs 双层合成；sCO2 f 侧重提
          K 面 + sco2_df cF 在新导出 + γ_f 重冻。tripwire 现绿，f 侧 CFD 基未动）
  - [ ] D-2c 方法对比矩阵（1-2 轮）：现行 γ 面 vs refit 基 vs 物理化（Ergun 族）vs
        贝叶斯标定，同考卷打分 + 外推稳健性（上海 16 盲 + 7-6 HX 盲 = 真考）
