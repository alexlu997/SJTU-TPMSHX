...
- [ ] 候选 D · D-F 系数获取方法（Alex 2026-07-20 提出；**2026-07-22 立项对话拍板边界，
  ...
  - [~] D-2c' 方法对比矩阵（`66b23dc`，iter 81）：**打分台已建，两种流向都算**
        ——production 4.88% / two_layer 25.29% / **two_layer_dir 4.92%**
        （cF 526.54 vs 534.80，max|e| 反而更好）/ smooth_only 67.49%（下界）。
        **534.8 基本可被预测出来**；γ 两层买了约 13 倍误差改善。
        诚实性：two_layer_dir 非纯盲考（流向因子取自考卷，与 production 同为
        1 个上海标量）。**Ergun 零参数变体补建**（`46310e5`，iter 82）：41.12%，
        **反优于 smooth_only 67.49%**——缺一层物理的精确 CFD 不如量级对的粗糙
        经验式。**贝叶斯标定变体补建**（`fd97d7c`，iter 83）：后验带诚实度检验——
        dir 方向 68/95% 覆盖 **81%/94%**（几乎精确），raw 方向 **0%/6%**
        ⇒ **同一条带加了流向因子才诚实，不加是"自信地错"**；带只含 HX 案间
        散差、不含流向这项系统不确定度。**四变体至此全建成**；**裁决仍待 D10**
...
- [x] **转按需模式（iter 84 执行，2026-07-25）**：候选 D 可做部分穷尽、剩余条目
      全部在等 D8-D12 人工答复，连报三轮无回应 ⇒ 按本条章程拆定时器
      （job `5e52a278` 已 CronDelete）。PROTOCOL/STATE/ROADMAP 原地保留；
      终审报告已补候选 D 章；决策简报 `upgrade/DECISION-BRIEF-2026-07-25.md`。
      **复活**：在 worktree 说一句"继续升级循环"即可（磁盘状态齐全，不丢进度）
