...
- [ ] 候选 D · D-F 系数获取方法（Alex 2026-07-20 提出；**2026-07-22 立项对话拍板边界，
  ...
  - [ ] D-0 溯源审计（1 轮）：col47 试件锚提取式钉到 file:line（闭式反演 or 穿求解器？
        水平口径入不入锚？）；534.8 完整出身；试验记录表可用试件数据盘点（除上海 16 外
        还有什么可标定）；新口径冻结清单（打靶 ON/F2/massflux/面提取/网格）；台账证伪史
        复核（rbf D7、L4/L5 弃锚、CF-REFIT 搁置因由）。产出 = 审计文档 + D-1..D-4 回填细化
  - [ ] D-1 考卷基建（1 轮）：固定考卷工具化一键跑分——上海 16（盲）+ D7 盲测 +
        sCO2 反向检验 + 水 CFD holdout + 外推压力测试（RBF 灾难史 = 必考题）；
        验证口径冻结声明（golden/BASELINE 在 D-3 前不动）
  - [ ] D-2a cF 光滑基升级（1-2 轮）：CF-REFIT 收尾——原始水 CFD 两段法提 cF
        （K 已做），log-TPS 面，与 SmoothDF 基同考卷对比
  - [ ] D-2b γ 重锚（1-2 轮）：纯试件锚（无上海点），Gyroid L 方向照 Diamond 模式重建；
        新口径（打靶 ON）下重跑考卷；γ 参数贝叶斯后验 + Δp 预测带（UQ 交付）
  - [ ] D-2c 方法对比矩阵（1-2 轮）：现行 γ 面 vs refit 基 vs 物理化（Ergun 族）vs
        贝叶斯标定，同考卷打分 + 外推稳健性
  - [ ] D-3 换默认提案（1 轮 + Alex 拍板）：证据表 → §5 `!` 重基准（含打靶默认翻转、
        golden air-B 工况点处置 D5(c)、README headline 重述为盲验证口径）
  - [ ] D-4 sCO2 翼（1-2 轮，排 D-3 后）：γ_Nu D 侧 1.75 入产线；γ_f hot/cold 锚选侧
        带方法论证据问 Alex；G 侧等 D_7_6/G_7_6 补算（触发器登记不阻塞）
